...
import static com.goodworkalan.prattle.PrattleException.*;
...
public class VariableProperties
{
    ...
    private String expand(String name, Set<String> seen)
    {
        ...
        {
            throw new PrattleException(PROPERTY_LOOP).add(name);
        }
        seen.add(name);
        ...
        if (value != null)
        {
            ...
            for (int i = 0, size = value.length(); i < size; i++)
            {
                ...
                case '\\':
                    ...
                    {
                        throw new PrattleException(TERMINAL_BACKSLASH).add(name);
                    }
                    break;
                case '$':
