...
function SlickGrid($container,data,columns,options)
{
	...
	function init() {
		...
		$divHeaders = $("<div style='width:100000px' />").appendTo($divHeadersScroller);
    ...
    function setColumn(column) {
        ...
        if (hidden || width)  {
	        var rule = $.rule("." + uid + " .grid-canvas .c" + i, "style");
            if (hidden) rule.css("display", columns[index].hidden ? "none" : "display");
            if (width) rule.css("display", columns[index].width + "px");
        }
		console.timeEnd("cssRules");
    }
